#include <stdio.h>
#include <stdlib.h>
#define NOMEFILE "easy_test_set.txt"
...
int disp_rip(int pos, int *pietre, int *scelte, int *sol, int n, int k, int max){
    int i,j;
    if(pos>=k){
        return is_valid(pietre,sol,k,n); 
    }
    for(i=0; i<n && max==-1; i++){
        if(is_valid(pietre,sol,pos,n)!=-1){ //pruning (controllo la validità della soluzione parziale)
            sol[pos] = scelte[i];
...
int is_valid(int *pietre, int *sol, int len, int n){
    ...
    for(i=0; i<len; i++){
        ...
        case 2:
            pietre_locale[2]--;
            if(((sol[i+1]!=0  && sol[i+1]!=1) && (i!=len-1)) || pietre_locale[2]<0){
                free(pietre_locale);
                return -1;
            }
            break;
        case 3:
            pietre_locale[3]--;
            if(((sol[i+1]!=2  && sol[i+1]!=3) && (i!=len-1)) || pietre_locale[3]<0){
                free(pietre_locale);
                return -1;
            }
            break;
